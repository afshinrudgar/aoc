...
main :: IO ()
main = do
  readLines "input/a2p1.txt" >>= print . solve
...
isLimitedTo :: Set -> Game -> Bool
isLimitedTo set game =
  all (<= red set) (reds game)
    && all (<= green set) (greens game)
    && all (<= blue set) (blues game)

isPossible :: Game -> Bool
isPossible = isLimitedTo (12, 13, 14)

solve :: [String] -> Int
solve = sum . map getGameId . filter isPossible . map readGame
