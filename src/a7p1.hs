...
main :: IO ()
main = do
  readFile "input/a7p1.txt" >>= print . solve
...
getHandType (c1, c2, c3, c4, c5)
  | snd (head itemCnt) == 5 = FiveOfAKind
  | snd (head itemCnt) == 4 = FourOfAKind
  | snd (head itemCnt) == 3 && snd (itemCnt !! 1) == 2 = FullHouse
  | snd (head itemCnt) == 3 = ThreeOfAKind
  | snd (head itemCnt) == 2 && snd (itemCnt !! 1) == 2 = TwoPairs
  | snd (head itemCnt) == 2 = OnePair
  | otherwise = HighCard
  where
    cnt' =
      foldl
